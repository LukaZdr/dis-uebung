...
public class Main {
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        ...
        List<RunnableOperation> operations = null;
        // (read lock) SELECT FROM sheet3 WHERE id = 3 FOR SHARE;
        // (exclusive lock) SELECT FROM sheet3 WHERE id = 3 FOR UPDATE;
        if (schedule == 1) {
            ...
            operations = new ArrayList<>(Arrays.asList(
                    ...
                    new RunnableOperation(c1, 'w', "UPDATE dissheet3 SET name = 'Foo' WHERE id = 2;"),
                    new RunnableOperation(c1, 'c', "COMMIT;"),
                    new RunnableOperation(c2, 'r', "SELECT name FROM dissheet3 WHERE id = 2;"),
                    new RunnableOperation(c2, 'w', "UPDATE dissheet3 SET name = 'Foo' WHERE id = 1;"),
                    new RunnableOperation(c2, 'w', "UPDATE dissheet3 SET name = 'Bar' WHERE id = 2;"),
                    new RunnableOperation(c2, 'c', "COMMIT;")));
        } else if (schedule == 4) {
            //S1 = r1(x) w2(x) c2 w1(x) r1(x) c1
            operations = new ArrayList<>(Arrays.asList(
        ...
        if (serializable) {
	        ...
	        operations.add(new RunnableOperation(c1, 't', "BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;"));
	        operations.add(new RunnableOperation(c2, 't', "BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;"));
	        Collections.reverse(operations);
	    }
